...
router.post('/login', async (req, res) => {
    try {
        ...
        // CORRECTION: Vérifier si l'utilisateur a un mot de passe défini
        if (!user.password) {
        ...
        res.json({ token, user, entreprise: user.entrepriseId || { nom: user.entreprise } });
    } catch (err) {
...
router.get('/verify-token', async (req, res) => {
    try {
        ...
        console.error('Erreur serveur verify-token:', err);
...
router.post('/invite', authenticateToken, async (req, res) => {
    try {
        ...
        const existingUser = await User.findOne({ $or: [{ email: newUserData.email }, { username: newUserData.username }] });
        if (existingUser) return res.status(400).json({ message: 'Email ou username déjà utilisé' });

        // Génération du resetToken pour que l'utilisateur définisse son mot de passe
        ...
        // CORRECTION: Ne pas définir de mot de passe lors de la création
        // newUserData.password sera undefined, ce qui est voulu
        ...
        res.status(201).json({ 
            ...
            user: newUser,
            debug: { tokenGenerated: !!resetToken, resetToken } 
        });
    } catch (err) {
...
router.post('/first-login', async (req, res) => {
    try {
        ...
        const user = await User.findOne({ 
            ...
            isActive: false // S'assurer que c'est bien un premier login
        });
        ...
        // CORRECTION: Générer un token JWT pour connecter automatiquement l'utilisateur
        ...
        res.json({ 
            ...
            token: jwtToken // Retourner le token pour connexion automatique
        });
    } catch (err) {
...
router.post('/request-reset-password', async (req, res) => {
    try {
        ...
        // CORRECTION: Vérifier si le compte est activé
...
router.post('/reset-password', async (req, res) => {
    try {
        ...
        if (!token || !newPassword || !email) return res.status(400).json({ success: false, message: 'Token, email et nouveau mot de passe requis' });

        const user = await User.findOne({ 
            email: email.toLowerCase(), 
            resetToken: token, 
            resetTokenExpiry: { $gt: Date.now() },
            isActive: true // Seuls les comptes déjà activés peuvent réinitialiser
        });
...
router.put('/:id', authenticateToken, preventCrossCompanyAccess, async (req, res) => {
    try {
        ...
        // CORRECTION: Ne pas permettre la modification du mot de passe via cette route
